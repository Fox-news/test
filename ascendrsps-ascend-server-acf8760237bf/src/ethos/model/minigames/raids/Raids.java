...
public class Raids {
	...
	private List <Player> raidTeam = new ArrayList<Player>();
	...
	//CODE ADDED AND EDITED BY FOX FOR NEW LOBBY SYSTEM
	
	private static void joinRaidLobby(Player player) {//attempt to join raid lobby area
		//conditions
		if(RaidConstants.raidLobby.size() > 21) {//Lobby will only hold max raid size
			player.sendMessage("Lobby is full, try again in a moment...");
			return;
		}
		if (player.combatLevel < 70) {//combat level requirment to join raid
			player.sendMessage("You need a combat level of atleast 70 to join raids.");
			return;
		}
		if(RaidConstants.raidLobby.size() == 0) {//if no players are in the lobby when you join the countdown timer starts
			startRaidTimer(player);
		}
		
		player.getRaids().getRaidTeam().clear();//gets rid of old raid team
		clearRaidVariables(player);//clears all old raid variables
		RaidConstants.raidLobby.add(player);//adds player to lobby
		player.getPA().movePlayer(3304,5194);//moves player to lobby
		player.sendMessage("You are now in the Raid lobby. The Raid will be starting soon.");
		
	}
	private static void clearRaidVariables(Player player) {//clears all raid variables for new raid
		player.getRaids().roomNames.clear();
		player.getRaids().roomPaths.clear();
...
private static void startRaidTimer(Player p) {//counts down when raid starts
	RaidConstants.lobbyActive = true;
	Timer timer = new Timer();
	TimerTask task = new TimerTask()
			
	
	{
	    int seconds = RaidConstants.raidLobbyTimer;
	    @Override
	    public void run()
	    {
	       RaidConstants.timeLeft++;
	    	
	       if(RaidConstants.timeLeft % seconds == 0) {
	    	   timer.cancel();
	    	   Raids raid = new Raids(p);
				RaidConstants.lobbyActive = false;
				raid.startRaid();
	    	  
	       }
	       else {
	    	// System.out.println("Time left:" + (seconds - (RaidConstants.timeLeft %seconds)) );
	...
	public static void drawInterface(Player p) {//draws the interface for how much time is left in lobby
		int seconds = RaidConstants.raidLobbyTimer;
		p.getPA().sendFrame126("Raid begins in: @gre@"+(seconds - (RaidConstants.timeLeft %seconds)), 6570);
 ...
 * Starts the raid.
 */
public void startRaid() {//Initializes the raid
	int memberCount = RaidConstants.raidLobby.size();//how many players in lobby

	if (memberCount > 22) {//max amount of allowed players in a raid at a time
		player.sendMessage("The lobby is full at the moment, try again in a minute.");
		return;
	}
	raidLeader = player;//gets active player
	int path1 = 1;
	int way1=Misc.random(1);
	path = path1;
	way=way1;
	raidPoints = 0;
	for (Player lobbyPlayer : RaidConstants.raidLobby) {//gets all players in lobby
		Player p = PlayerHandler.getPlayer(lobbyPlayer.getName());
		if (p == null) {
			continue;
		}
		p.getRaids().raidLeader = player;//used so players can go to next room without raid leader
		p.getRaids().path = path1;
		p.getRaids().way= way1;
		p.getRaids().raidPoints=0;
		p.getRaids().getRaidTeam().clear();
		p.getRaids().setRaidTeam(RaidConstants.raidLobby);//turns lobby into team
		...
		//moves players to instances
		p.getRaids().updateRaidPoints();
		p.getPA().movePlayer(getStartLocation().getX(),getStartLocation().getY(),getHeight(player));
		p.sendMessage("@red@The raid has now started! Good Luck!");
		
	}//end of loop
	//after lobby is teamed together and put into a team this clears the lobby for more players to enter
	RaidConstants.raidLobby = new ArrayList<Player>();
}
	...
	public void leaveGame(Player player) {
		...
		roomNames.clear();//cleared array instead of turning it null
		roomPaths.clear();//clean
		currentRoom = 0;
		mobAmount=0;
		reachedRoom = 0;
		raidLeader=null;
	...
	public void handleMobDeath(int npcType) {
		...
		player.getRaids().raidLeader.getRaids().mobAmount-=1;//brb
		switch(npcType) {
			case OLM:
				...
				for (Player team : player.getRaids().raidTeam) {
					Player p = team;
					if (!p.inRaids()) {//makes it so you have to be in olm room for reward
						continue;
					}
					if(p.inOlmRoom() == true) {
						p.getRaids().giveReward();
						p.getItems().addItemUnderAnyCircumstance(p.raidReward[0][0], p.raidReward[0][1]);
					} else if (p.inOlmRoom() == false){
						p.sendMessage("You were not in the Olms room by end of raid so you recieve no reward.");
					}
		...
		if(player.getRaids().raidLeader.playerName != player.playerName) {
				if (player.getRaids().currentRoom + 1 > player.getRaids().raidLeader.getRaids().reachedRoom && currentRoom != 0) {
					if (player.getRaids().raidLeader.getRaids().mobAmount > 0 && player.getRaids().currentRoom == player.getRaids().raidLeader.getRaids().currentRoom) {
						player.sendMessage("@red@Please defeat all the monsters before going to the next room1.");
						return;
					}
				}
			...
			if (player.getRaids().currentRoom == player.getRaids().raidLeader.getRaids().reachedRoom) {
				if (player.getRaids().raidLeader.getRaids().mobAmount > 0) {
					player.sendMessage("@red@Please defeat all the monsters before going to the next room2");
					System.out.println(player.getRaids().currentRoom+"   "+player.getRaids().raidLeader.getRaids().reachedRoom);
					return;
				}
			}
		}
...
**/
	...
	public List<Player> getRaidTeam() {
		return raidTeam;
	}

	public void setRaidTeam(List<Player> raidTeam) {
