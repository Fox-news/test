 ...
 */
	...
	public void poke(Player player, NPC npc) {
		refreshInstance();
		if(player.heightLevel == 0) {//if player is not in a instance
	...
	public void executeAttack(Player p,int type) {//executes the different type of attacks
		...
		case 7://spit fire attack that shoots rapid
			...
			Server.getGlobalObjects().add(new GlobalObject(11700, playerX, playerY, player.getHeight(), 0, 10, 30, -1));//poision cloud
	...
	public void spawnCrab() {
		if(middleOfVorkath(player) == true) {//if player is in the middle of the map the crab spawns on Y Axis
			Server.npcHandler.spawnNpc(player, 8062, player.getX(), player.getY()+8, player.getHeight(), 1, 100, 200, 200, 200, true, true);
		return;
		}
		/*
		...
		if(rightOfVorkath(player) == true) {
			Server.npcHandler.spawnNpc(player, 8062, player.getX()-8, player.getY(), player.getHeight(), 1, 100, 200, 200, 200, true, true);
			} else {
				Server.npcHandler.spawnNpc(player, 8062, player.getX()+8, player.getY(), player.getHeight(), 1, 100, 200, 200, 200, true, true);
			}
	}
