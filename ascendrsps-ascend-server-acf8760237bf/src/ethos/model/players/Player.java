...
import ethos.model.players.skills.Fishing2;
...
public class Player extends Entity {
	...
	public int currentRender = 0;//the current render for the player, increments to 16 and stops. 16 is max
	 ...
	 */
public boolean renderingNpcs = false;//when player is logging in npcs dont render until player is on all the way
	...
	private Fishing2 fishing2 = new Fishing2(this);//creates custom VORKATH instance for player
	...
	private DailyGearBox dailyRewardBox = new DailyGearBox(this);
	...
	public Player raidLeader;//gets players raidLeader
	public int raidInstance;//gets instance player is in
	...
	public void destruct() {
		...
		if (Vorkath.inVorkath(this)) {//vorkath locations
	...
	public void initialize() {
		try {
			...
			if (tutorial.isActive() == false) {
				System.out.println("not making it here?");
			dailyReward.getDay();
			}
			dailyRewardBox.startEvent();
	...
	public void logout() {
		...
		if (Vorkath.inVorkath(this)) {//if player is in vorkath and log out they get moved
	...
	public void process() {
		...
		}  else if (this.inRaidLobby() == true && this.getRaids().getRaidTeam().size() == 0) {
			getPA().walkableInterface(6673);
			Raids.drawInterface(this);
	...
	public Fishing2 getFishing2() {//VORKATH player instance
		return fishing2;
	}
	...
	public DailyGearBox getDailyRewardBox() {
	...
	public boolean inRaids() {//checks to see if player is in the raid
		return (absX > 3210 && absX < 3368 && absY > 5137 && absY < 5759);
	}
	public boolean inOlmRoom() {//checks to see if player is in olm room
		return (absX > 3225 && absX < 3240 && absY > 5730 && absY < 5750);
	}
	public boolean inShamanRoom() {//checks to see if player is in olm room
		return (absX > 3300 && absX < 3324 && absY > 5253 && absY < 5276);
	}
	public boolean inMysticRoom() {//checks to see if player is in olm room
		return (absX > 3300 && absX < 3320 && absY > 5248 && absY < 5274);
	}
	public boolean inVasaRoom() {//checks to see if player is in olm room
		return (absX > 3299 && absX < 3323 && absY > 5279 && absY < 5308);
	}
	public boolean inVanguardRoom() {//checks to see if player is in olm room
		return (absX > 3307 && absX < 3320 && absY > 5311 && absY < 5341);
	}
	public boolean inDemonRoom() {//checks to see if player is in olm room
		return (absX > 3300 && absX < 3319 && absY > 5354 && absY < 5369);
	}
	public boolean inMutadileRoom() {//checks to see if player is in olm room
		return (absX > 3300 && absX < 3316 && absY > 5312 && absY < 5337);
	}
	public boolean inTektonRoom() {//checks to see if player is in olm room
		return (absX > 3300 && absX < 3320 && absY > 5282 && absY < 5306);
	}
	public boolean inRaidLobby() {//checks to see if player is in the raid lobby
		if(absX >= 3290 && absX <= 3320 && absY >= 5180 && absY <= 5206)  {
			return true;
		}
		return false;
	}
	...
	public boolean withinDistance(NPC npc) {//npc rendering || return statement is increased in slow increments to stop overload bugs
		if (heightLevel != npc.heightLevel)
			return false;
		if (npc.needRespawn == true)
			return false;
		int deltaX = npc.absX - absX, deltaY = npc.absY - absY;
		return deltaX <= currentRender && deltaX >= -currentRender-1 && deltaY <= currentRender && deltaY >= -currentRender-1;
	}
	...
	public void setRaidLeader(Player raidLeader) {
		this.raidLeader = raidLeader;
	}

	public int getRaidInstance() {
		return raidInstance;
	}

	public void setRaidInstance(int raidInstance) {
